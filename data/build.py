...
def get_dataset_args(args):
    ...
    if args.model in {"VideoMAE", "TransferOT", "FusionModel"}:
        params.update(
            transform=VideoMAEImageProcessor.from_pretrained(
                args.pretrained_model_visual
            )
        )
    else:
        params.update(
...
def get_dataloader(dataset, train_idx, test_idx, args):
    ...
    train_loader = DataLoader(
        ...
        num_workers=0,
        collate_fn=reg_collate if args.memory_sufficient else af_collate_fn,
        drop_last=True,
    ...
    test_loader = DataLoader(
        ...
        num_workers=0,
        collate_fn=reg_collate if args.memory_sufficient else af_collate_fn,
        drop_last=not args.memory_sufficient,
