...
@DATASETS.register("DOLOS")
class DOLOS_Dataset(Dataset):
    def __init__(
        self,
        dataset_path,
        transform,
        ...
        Args:
            path (str): path to DOLOS dataset
        """
        super(DOLOS_Dataset, self).__init__()
        ...
        for tag in sorted(self.tags):
            ...
            _files = [file.replace("Clips", "Audios")[:-3] + "wav" for file in _files]
    ...
    def memory_sufficient(self, args):
        ...
        if args.model in ["ViT_model", "PECL"]:
            ...
            suffix = "_VideoMAE"
        if not os.path.exists(os.path.join("cache", f"DOLOS{suffix}.pkl")):
            self.clips = []
            ...
            if not os.path.exists("cache"):
                os.makedirs("cache")
            with open(os.path.join("cache", f"DOLOS{suffix}.pkl"), "wb") as f:
                pickle.dump(self.clips, f)
        else:
            with open(os.path.join("cache", f"DOLOS{suffix}.pkl"), "rb") as f:
                self.clips = pickle.load(f)
        if args.model in ["PECL"]:
            suffix = "_ViT"
        else:
            suffix = ""
        if not os.path.exists(os.path.join("cache", f"DOLOS-audio{suffix}.pkl")):
            self.audios = []
            for audio_file in tqdm(self.audio_files):
                audio = self.read_aud(audio_file)
                self.audios.append(audio)
            self.audios = af_pad_sequence(self.audios)
            with open(os.path.join("cache", f"DOLOS-audio{suffix}.pkl"), "wb") as f:
                pickle.dump(self.audios, f)
        else:
            with open(os.path.join("cache", f"DOLOS-audio{suffix}.pkl"), "rb") as f:
