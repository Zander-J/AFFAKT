...
def evaluate(model, test_loader, amp, tbar):
    ...
    for data_collection in test_loader:
        ...
        if res.transport_plan is not None:
            tp_meter.update(res.transport_plan, label.size(0))       
        if tbar is not None:
            if res.transport_plan is not None:
                tbar.set_postfix(tp="%.4f" % tp_meter.avg)
            tbar.update()
    torch.cuda.synchronize()
...
def train(args):
    ...
    if args.memory_sufficient:
        if local_rank == 0:
            log.info("Memory sufficient, load video now. Use cached data in 'cache/'.")
        dataset.memory_sufficient(args)

    if local_rank == 0:
        index_file = None
        if args.write_to_local:
            index_file = h5py.File(
                os.path.join(args.output_dir, "train_test_index.hdf5"), "w"
            )
            index_file.create_dataset(
                "Name", shape=(1,), data=args.dataset, dtype="S10"
            )
    ...
    for fold_idx, (train_set, test_set) in enumerate(
        ...
        if args.distributed:
            if local_rank == 0:
                log.info(
                    f"Using {torch.cuda.device_count()} {torch.cuda.get_device_name()} GPUs."
                )
        ...
        for epoch in range(args.num_epochs):
            ...
            loss_train, acc_train = train_one_epoch(
                model, train_loader, optim, args.amp, tbar
            )
            if (epoch + 1) % PRINT_STEP == 0:
                ...
                loss_test, (f1_test, acc_test, auc_test) = evaluate(model, test_loader, args.amp, tbar)
            if args.distributed:
                torch.distributed.barrier()
        ...
        if local_rank == 0:
            ...
            if args.write_to_local and args.save_final_ckpt:
                save_model(
                    model,
                    os.path.join(
                        args.output_dir, "checkpoints", f"fold_{fold_idx}_final.pth"
                    ),
                    args.model,
                )
            ...
            try:
                a.append(model.module.prototype.cpu())
            except:
                try:
                    a.append(model.modules.prototype.cpu())
                except:
                    pass
    if local_rank == 0:
        if args.write_to_local:
        ...
        try:
            all_acc = [
                summary[key]["best_acc"] for key in summary.keys() if "fold" in key
            ]
            summary["acc_mean"] = np.mean(all_acc)
            summary["acc_std"] = np.std(all_acc)
            all_f1 = [
                summary[key]["best_f1"] for key in summary.keys() if "fold" in key
            ]
            summary["f1_mean"] = np.mean(all_f1)
            summary["f1_std"] = np.std(all_f1)
            all_auc = [
                summary[key]["best_auc"] for key in summary.keys() if "fold" in key
            ]
