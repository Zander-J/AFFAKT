...
> Ver車nica P谷rez-Rosas, Mohamed Abouelenien, Rada Mihalcea, and Mihai Burzo. 2015. Deception Detection using Real-life Trial Data. In Proceedings of the 2015 ACM on International Conference on Multimodal Interaction (ICMI '15). Association for Computing Machinery, New York, NY, USA, 59每66. https://doi.org/10.1145/2818346.2820758
...
- Step 1: Clone the [MAE-DFER](https://github.com/sunlicai/MAE-DFER) code and download the checkpoints.
    > git clone https://github.com/sunlicai/MAE-DFER.git

    Then, follow their instructions to reproduce feature embeddings of VFER datasets.

- Step 2: Save the *feature embeddings*, *ground truth labels* and *prediction probabilities* to local HDF5 file. The processed HDF5 file should be like:
    ```python
    ...
    ```
- Step 3: Specify the path to these files by `--source_feature` in `config.py` or `$source_feature` in `train.sh`.

## Pre-process data (optional)
It is pretty slow to process video data online. Thus, if you have sufficient CPU memory, it is recommended to set `--memory_sufficient` to `True` to pre-process the video data offline. Then the pre-processed data could be found at `cache/`. 
