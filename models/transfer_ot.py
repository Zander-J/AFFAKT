...
class TransferOT(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.result = namedtuple("Result", ["output", "loss", "transport_plan"])
    ...
    def forward(self, data_collection, train_stage=False):
        ...
        if self.trans:
            if self.transfer_method == "HOTKT":
                ...
                res = self.result(output=output, loss=loss, transport_plan=tp)
                return res
            elif self.transfer_method == "ADC":
                cost_inner, pi, _ = self.sinkhorn_low_level(
                    self.source_feature.to(self.dtype),
                    target_feature.unsqueeze(0),
                    self.sample_probabi,
                )
                _, pi, _ = self.sinkhorn_high_level(
                    self.source_mean.to(self.dtype), target_feature, cost_inner
                )
                pi = pi.permute(1, 0).to(self.dtype) * len(label)
                tran = (pi.unsqueeze(-1) * self.source_mean.to(self.dtype).unsqueeze(0)).sum(axis=1)
                output = self.classifier(0.8 * target_feature + 0.2 * tran)
                loss = self.loss_cls(output, label)
                res = self.result(output=output, loss=loss, transport_plan=pi)
                return res
            else:
            ...
            loss = self.loss_cls(output, label)
            res = self.result(output=output, loss=loss, transport_plan=None)
    ...
    def select_prototype(self, pi):
        k = torch.cdist(pi, self.prototype).argmax(axis=1)
